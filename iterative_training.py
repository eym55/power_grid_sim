...
#TODO import attacker and defender games
...
class Agent:
    ...
    def get_q_values(self,state):
        state = np.reshape(state, [1, self.state_dim])
        q_values = self.target_model.predict(state)[0]
        return q_values
...
def main():
    ...
    #TODO
  #  for _ in 1000:
    # build defender game (attack ditribution)
    # train defender
    # defender q_values= agent.get_q_values
    # build attacker game (attack ditribution)
    # train attacker
    # attack distribution = agent.get_q_values
    ...
    #Initalize random attack
    attack_distribution =  np.random.dirichlet(np.ones(LINES),size= 1)[0]
    #Build defender game
    env = PowerGrid(network,attack_distribution)
    #initaliaize a defender agent
    defender_agent = Agent(env)
    #Train the agent and store the results
    history = defender_agent.train(max_episodes=10)
    #Saving results
