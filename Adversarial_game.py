...
class PowerGrid(gym.Env):
  ...
  def __init__(self, network: Network, defend_distribution,timesteps = 10):
    ...
    self.defend_distribution = defend_distribution
    ...
    # Actions are attack line, each action correspoonds to the index of the line to attack.
  ...
  def step(self, action):
    ...
    #Sample from defend distribution until we get a line thats not removed
    defender_action = None
    while defender_action in self.removed_lines:
      defender_action = np.random.choice(self.NUM_LINES,p = self.defend_distribution)
    # If not defended, remove line and update network
    if action == defender_action:
        lopf_status = ('ok',None)
    else:
        lopf_status = self._apply_attack(defender_action)
